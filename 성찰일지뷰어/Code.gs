...
function doGet(e) {
  ...
  try {
    ...
    } else if (action === 'debugAudience') {
      data = (function() {
        try {
          const ss = SpreadsheetApp.openById(SS_AUDIENCE);
          const sh = ss.getSheets()[0];
          const d  = sh.getDataRange().getValues();
          return { header: d[0], row2: d[1]||[], row3: d[2]||[], total: d.length };
...
function getAudienceStats(studentName, ban, anonymous) {
  ...
  try {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      const wBan  = wBanCol >= 0 ? norm(row[wBanCol] || '') : '';
      if (!wName) continue;
...
function getMyData(ban, num, name) {
  ...
  // 청취자 평가 (받은 것 익명)
  const audience = getAudienceStats(name, ban, true);

  return { reflections, linksData, peer, audience, scores, modum };
}
